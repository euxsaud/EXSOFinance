...
export const ALPINE_KPIS = () => ({
	...
	filter_SetDate(event) {
		...
		if (value.length === 2) {
			...
			const date = dateFns.parseISO(value[0]);
			this.filter.date.from = dateFns.startOfMonth(date);
			this.filter.date.to = dateFns.endOfMonth(date);
		}

		this.filter.date.formattedFrom = dateFns.format(this.filter.date.from, 'MMMM dd, yyyy');
		this.filter.date.formattedTo = dateFns.format(this.filter.date.to, 'MMMM dd, yyyy');
	...
	chart_HistoryBalance() {
		...
		this.BKDATA.forEach(({ date, transaction, currency, amount }) => {
			...
			const startMonth = dateFns.startOfMonth(date);

			// Set formatted date like key
			const key = dateFns.format(startMonth, 'MM-01-yyyy');
		...
		const chartData = Object.values(groupedData).map((item) => {
			const date = dateFns.format(item.date, 'MMM yyyy');
	...
	async init() {
		...
		this.opts.transactions = getOptions.transactionMethods;
